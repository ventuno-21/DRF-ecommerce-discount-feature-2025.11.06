...
class ProductVariant(models.Model):
    ...
    currency = models.ForeignKey(
        "Currency", on_delete=models.PROTECT, related_name="vendor_products"
    )
...
class Currency(models.Model):
    """
    Represents a supported currency (USD, EUR, IRR, etc.)
    """

    code = models.CharField(
        max_length=3, unique=True, help_text="ISO code, e.g., USD, EUR, IRR"
    )
    name = models.CharField(max_length=50)
    symbol = models.CharField(
        max_length=5, blank=True, help_text="Currency symbol like $, €, ﷼"
    )
